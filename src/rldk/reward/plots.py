...
def generate_calibration_plots(reward_data: pd.DataFrame,
                              reward_col: str,
                              human_pref_col: Optional[str] = None,
                              ground_truth_col: Optional[str] = None,
                              output_path: Optional[Path] = None,
                              figsize: tuple = (12, 8)) -> Dict[str, Any]:
    """
    ...
    ax4 = axes[1, 1]
    if human_pref_col and human_pref_col in reward_data.columns:
        _plot_correlation(ax4, reward_data, reward_col, human_pref_col, 'Human Preference')
    elif ground_truth_col and ground_truth_col in reward_data.columns:
        _plot_correlation(ax4, reward_data, reward_col, ground_truth_col, 'Ground Truth')
    else:
...
def _plot_reward_distribution(ax: plt.Axes, reward_values: pd.Series):
    ...
    if len(clean_values) == 0:
        ax.text(0.5, 0.5, 'No valid reward data', ha='center', va='center', transform=ax.transAxes)
...
def _plot_calibration_curve(ax: plt.Axes, data: pd.DataFrame, reward_col: str, human_pref_col: str):
    ...
    if len(valid_data) < 10:
        ax.text(0.5, 0.5, 'Insufficient data for calibration', ha='center', va='center', transform=ax.transAxes)
    ...
    reward_normalized = (reward_scores - reward_scores.min()) / (reward_scores.max() - reward_scores.min())
    ...
    try:
        ...
        ax.plot(mean_predicted_value, fraction_of_positives, 'o-', label='Calibration Curve')
        ...
        ax.text(0.5, 0.5, f'Calibration failed: {str(e)}', ha='center', va='center', transform=ax.transAxes)
        ax.set_title('Calibration Curve')


def _plot_correlation(ax: plt.Axes, data: pd.DataFrame, reward_col: str, other_col: str, other_name: str):
    ...
    p = np.poly1d(z)
    ax.plot(valid_data[reward_col], p(valid_data[reward_col]), "r--", alpha=0.8, linewidth=2)
...
def generate_drift_plots(drift_metrics: pd.DataFrame,
                        output_path: Optional[Path] = None,
                        figsize: tuple = (12, 8)) -> Dict[str, Any]:
    """
...
def _plot_drift_timeline(ax: plt.Axes, drift_metrics: pd.DataFrame):
    ...
    if step_metrics.empty:
        ax.text(0.5, 0.5, 'No temporal drift data', ha='center', va='center', transform=ax.transAxes)
        ax.set_title('Drift Timeline')
        return
    
    # Plot drift events
    ax.scatter(step_metrics['step'], step_metrics['effect_size'], 
               c=step_metrics['p_value'], cmap='viridis', alpha=0.7, s=50)
...
def _plot_effect_size_distribution(ax: plt.Axes, drift_metrics: pd.DataFrame):
    ...
    ax.axvline(mean_es, color='red', linestyle='--', alpha=0.8, label=f'Mean: {mean_es:.3f}')
    ax.legend()
...
def _plot_drift_types(ax: plt.Axes, drift_metrics: pd.DataFrame):
    ...
    if 'drift_type' not in drift_metrics.columns:
        ax.text(0.5, 0.5, 'No drift type data', ha='center', va='center', transform=ax.transAxes)
    ...
    ax.pie(drift_counts.values, labels=drift_counts.index, autopct='%1.1f%%', startangle=90)
