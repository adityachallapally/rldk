...
def analyze_calibration(run_data: pd.DataFrame, 
                       reward_col: str,
                       threshold: float = 0.7) -> Tuple[float, Dict[str, Any]]:
    """
    ...
    if 'human_preference' in run_data.columns:
        ...
        score, gt_details = _analyze_ground_truth_calibration(
            run_data, reward_col
        )
...
def _analyze_human_preference_calibration(run_data: pd.DataFrame, 
                                        reward_col: str) -> Tuple[float, Dict[str, Any]]:
...
def _analyze_ground_truth_calibration(run_data: pd.DataFrame, 
                                     reward_col: str) -> Tuple[float, Dict[str, Any]]:
...
def calculate_calibration_metrics(reward_scores: np.ndarray, 
    ...
    try:
        ...
        scores_normalized = (reward_scores - reward_scores.min()) / (reward_scores.max() - reward_scores.min())
