...
from .plots import generate_calibration_plots
...
def health(run_data: pd.DataFrame, 
           ...
           threshold_leakage: float = 0.3) -> RewardHealthReport:
    """
    ...
    if reference_data is not None:
        ...
        if drift_detected:
            ...
            fixes.append("Check for data distribution shifts or model architecture changes")
...
def _detect_saturation(reward_values: pd.Series, threshold: float) -> Tuple[List[str], Dict[str, Any]]:
    ...
    if upper_ratio > threshold:
        issues.append(f"High upper saturation: {upper_ratio:.1%} of rewards at upper bound")
    
    if lower_ratio > threshold:
        issues.append(f"High lower saturation: {lower_ratio:.1%} of rewards at lower bound")
...
def _detect_shortcut_signals(run_data: pd.DataFrame, reward_col: str, threshold: float) -> Tuple[List[str], Dict[str, float]]:
    ...
    if 'repetition_penalty' in run_data.columns:
        rep_corr = np.corrcoef(run_data['repetition_penalty'], run_data[reward_col])[0, 1]
    ...
    if 'has_markdown' in run_data.columns:
        markdown_corr = np.corrcoef(run_data['has_markdown'], run_data[reward_col])[0, 1]
    ...
    if 'keyword_count' in run_data.columns:
        keyword_corr = np.corrcoef(run_data['keyword_count'], run_data[reward_col])[0, 1]
...
def _detect_label_leakage(run_data: pd.DataFrame, reward_col: str, threshold: float) -> float:
    ...
    for col in suspicious_cols:
        if col in run_data.columns:
            ...
            try:
                corr = np.corrcoef(run_data[col].astype(float), run_data[reward_col])[0, 1]
                if not np.isnan(corr) and abs(corr) > threshold:
                    leakage_risk += 0.3  # Increment risk for each suspicious correlation
