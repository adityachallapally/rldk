...
def generate_determinism_card(
    ...
    card_data = {
            ...
            "replicas": int(len(set(event.rng.get("seed") for event in events if event.rng.get("seed") is not None))),
            "metrics_compared": list(events[0].metrics.keys()) if events else [],
            "replica_variance": _calculate_replica_variance(events),
            "rng_map": {
                "python_hash_seed": events[0].rng.get("python_hash_seed") if events else None,
                "torch_deterministic": bool(determinism_result["flags"]["cudnn_deterministic"]),
                "torch_seed": "Set per replica" if rng_analysis["multiple_seeds"] else str(events[0].rng.get("torch_seed")) if events and events[0].rng.get("torch_seed") is not None else None,
                "numpy_seed": "Set per replica" if rng_analysis["multiple_seeds"] else str(events[0].rng.get("numpy_seed")) if events and events[0].rng.get("numpy_seed") is not None else None,
                "random_seed": "Set per replica" if rng_analysis["multiple_seeds"] else str(events[0].rng.get("random_seed")) if events and events[0].rng.get("random_seed") is not None else None
            },
...
def _analyze_rng_consistency(events: List[Event]) -> Dict[str, Any]:
    ...
    seeds = [event.rng.get("seed") for event in events if event.rng.get("seed") is not None]
...
def _generate_determinism_visualization(card_data: Dict[str, Any], output_path: Path) -> None:
    """Generate a visual representation of the determinism card."""
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(12, 10))
    fig.suptitle(f"Determinism Card - {card_data['run_id']}", fontsize=16, fontweight='bold')
    
    # Overall status
    status_color = 'green' if card_data['passed'] else 'red'
    status_text = 'PASS' if card_data['passed'] else 'FAIL'
    
    ax1.text(0.5, 0.5, status_text, fontsize=48, ha='center', va='center', 
    ...
    if card_data['replica_variance']:
        ...
        bars = ax3.bar(metrics, variances, color=['red' if v > 0.05 else 'green' for v in variances])
        ax3.set_title('Metric Variance', fontsize=14, fontweight='bold')
        ax3.set_ylabel('Variance')
        ax3.tick_params(axis='x', rotation=45)
        
        # Add threshold line
        ax3.axhline(y=0.05, color='orange', linestyle='--', alpha=0.7, label='Threshold')
        ax3.legend()
    else:
    ...
    issues_text = '\n'.join([f"• {issue}" for issue in issues[:3]]) if issues else "No issues detected"
    fixes_text = '\n'.join([f"• {fix}" for fix in fixes])
    
    ax4.text(0.05, 0.95, 'Issues:', fontsize=12, fontweight='bold', va='top', transform=ax4.transAxes)
    ax4.text(0.05, 0.85, issues_text, fontsize=9, va='top', transform=ax4.transAxes)
    ax4.text(0.05, 0.45, 'Fixes:', fontsize=12, fontweight='bold', va='top', transform=ax4.transAxes)
