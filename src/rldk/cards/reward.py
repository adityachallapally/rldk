...
def generate_reward_card(
    ...
    recommendations = _generate_reward_recommendations(
        reward_analysis, drift_detected, calibration_score, 
        saturation_detected, shortcut_signals, label_noise
    )
    ...
    card_data = {
        ...
        "passed": bool(_evaluate_reward_health(reward_analysis, drift_detected, calibration_score)),
...
def _generate_reward_visualization(card_data: Dict[str, Any], output_path: Path) -> None:
    ...
    bars = ax2.bar(metric_names, metric_values, 
                   color=['green' if v > 0.8 else 'orange' if v > 0.5 else 'red' for v in metric_values])
    ...
    ax3.text(0.5, 0.7, f"Calibration Score:\n{calib_score:.2f}", 
             fontsize=16, ha='center', va='center', fontweight='bold')
    ax3.text(0.5, 0.3, f"Drift Detected:\n{'Yes' if drift_detected else 'No'}", 
    ...
    ax4.text(0.05, 0.45, 'Recommendations:', fontsize=12, fontweight='bold', va='top', transform=ax4.transAxes)
