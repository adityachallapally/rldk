...
def scan_ppo_events(events: Iterator[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for event in events_list:
        ...
        grad_norm_policies.append(event.get("grad_norm_policy", event.get("policy_grad_norm", 0.0)))
        grad_norm_values.append(event.get("grad_norm_value", event.get("value_grad_norm", 0.0)))
    ...
    grad_rules = detect_gradient_ratio_anomalies(steps, grad_norm_policies, grad_norm_values)
    rules_fired.extend(grad_rules)
    
    # Rule 4: Advantage sanity checks
    advantage_rules = detect_advantage_anomalies(steps, advantage_means, advantage_stds, entropies, kl_values)
...
def detect_kl_controller_stuck(steps: List[int], kl_values: List[float], kl_coefs: List[float]) -> List[Dict[str, Any]]:
    ...
    for i in range(len(kl_outside_target) - 1):
        ...
        if next_idx == current_idx + 1:
            coef_change = abs(kl_coefs[next_idx] - kl_coefs[current_idx]) / max(kl_coefs[current_idx], 1e-8)
...
def detect_gradient_ratio_anomalies(steps: List[int], grad_norm_policies: List[float], grad_norm_values: List[float]) -> List[Dict[str, Any]]:
    ...
    for i, (step, policy_norm, value_norm) in enumerate(zip(steps, grad_norm_policies, grad_norm_values)):
...
def detect_advantage_anomalies(steps: List[int], advantage_means: List[float], advantage_stds: List[float], 
                              entropies: List[float], kl_values: List[float]) -> List[Dict[str, Any]]:
...
def compute_stats(kl_values: List[float], grad_norm_policies: List[float], 
                 grad_norm_values: List[float], entropies: List[float]) -> Dict[str, Any]:
