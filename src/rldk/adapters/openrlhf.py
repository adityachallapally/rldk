...
class OpenRLHFAdapter(BaseAdapter):
    ...
    def can_handle(self) -> bool:
        ...
        if self.source.is_file():
            ...
            openrlhf_files = list(self.source.glob("*.log")) + list(self.source.glob("*.jsonl"))
            ...
            if file_path.suffix == '.jsonl':
                with open(file_path, 'r') as f:
                    ...
                    if first_line:
                        ...
                        return ('openrlhf' in str(data).lower() or 
                                'rlhf' in str(data).lower() or
    ...
    def load(self) -> pd.DataFrame:
        ...
        if self.source.is_file():
            ...
            log_files = list(self.source.glob("*.log")) + list(self.source.glob("*.jsonl"))
    ...
    def _extract_openrlhf_metric(self, data: Dict[str, Any], line_num: int) -> Dict[str, Any]:
        """Extract metric from OpenRLHF JSON data."""
        metric = {
            'step': data.get('step', line_num),
            'phase': data.get('phase', 'train'),
            'reward_mean': data.get('reward_mean') or data.get('reward', {}).get('mean'),
            'reward_std': data.get('reward_std') or data.get('reward', {}).get('std'),
            'kl_mean': data.get('kl_mean') or data.get('kl_div', {}).get('mean'),
            'entropy_mean': data.get('entropy_mean') or data.get('entropy', {}).get('mean'),
