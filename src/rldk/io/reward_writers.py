...
def write_reward_health_card(report: RewardHealthReport, output_dir: Path) -> None:
    ...
    with open(report_path, 'w') as f:
        ...
        if report.passed:
            ...
            f.write("The reward model appears to be healthy with no significant pathologies detected.\n\n")
        else:
            f.write("## 🚨 Reward Health Issues Detected\n\n")
            f.write("The reward model shows potential pathologies that should be investigated.\n\n")
        ...
        if report.label_leakage_risk > 0.3:
            ...
            f.write("The reward model may have access to training signals it shouldn't see.\n")
...
def write_reward_health_summary(report: RewardHealthReport, output_dir: Path) -> None:
    ...
    if report.drift_metrics is not None and not report.drift_metrics.empty:
        summary['drift_summary'] = {
            ...
            'drift_types': report.drift_metrics['drift_type'].value_counts().to_dict() if 'drift_type' in report.drift_metrics.columns else {},
            'mean_effect_size': report.drift_metrics['effect_size'].mean() if 'effect_size' in report.drift_metrics.columns else np.nan,
            'max_effect_size': report.drift_metrics['effect_size'].max() if 'effect_size' in report.drift_metrics.columns else np.nan
        }
...
def generate_reward_health_report(report: RewardHealthReport, 
