...
from rldk.io.schemas import validate, DeterminismCardV1, PPOScanReportV1, CkptDiffReportV1
...
def compare_runs(
    ...
    try:
        ...
        if comparison["earliest_divergent_step"]:
            typer.echo(f"Earliest divergent step: {comparison['earliest_divergent_step']}")
...
def diff_ckpt(
    ...
    try:
        typer.echo(f"Comparing checkpoints:")
        ...
        typer.echo(f"L2 norm percentiles - 5%: {summary['l2_p05']:.6f}, 50%: {summary['l2_p50']:.6f}, 95%: {summary['l2_p95']:.6f}")
        
        if report["top_movers"]:
            typer.echo(f"\nTop parameter changes:")
            for i, mover in enumerate(report["top_movers"][:5]):
                typer.echo(f"  {i+1}. {mover['name']}: L2={mover['l2']:.6f}, cosine={mover['cosine']:.4f}")
...
def env_audit(
    ...
    try:
        ...
        typer.echo(f"  Determinism card: rldk_reports/determinism_card.json")
        typer.echo(f"  Lock file: rldk.lock")
        ...
        if determinism_card["nondeterminism_hints"]:
            typer.echo(f"  Nondeterminism hints: {len(determinism_card['nondeterminism_hints'])}")
...
def log_scan(
    ...
    try:
        ...
        if rules_fired:
            typer.echo(f"Anomalies detected:")
            for rule in rules_fired:
                typer.echo(f"  - {rule['rule']}: {rule['description']}")
                if rule.get("step_range"):
                    typer.echo(f"    Steps: {rule['step_range'][0]} to {rule['step_range'][1]}")
        else:
            typer.echo(f"No anomalies detected.")
...
def doctor(
    ...
    try:
        ...
        typer.echo(f"Files generated:")
        typer.echo(f"  - rldk_reports/determinism_card.json")
        typer.echo(f"  - rldk_reports/ppo_scan.json")
        typer.echo(f"  - rldk.lock")
        ...
        if env_healthy and log_healthy:
            ...
            typer.echo(f"\n⚠️  Issues detected:")
            if not env_healthy:
                typer.echo(f"  - Environment has nondeterminism issues")
            if not log_healthy:
                typer.echo(f"  - Training logs show {len(scan_report.get('rules_fired', []))} anomalies")
