...
def analyze_training_metrics(metrics_file: str) -> Dict:
    ...
    try:
        ...
        return {"steps": len(metrics), "step_47_kl": step_47_kl, "reward_range": reward_range}
...
def run_rldk_analysis(output_dir: str) -> bool:
    ...
    health_cmd = f"rldk reward-health --run {output_dir} --output-dir {output_dir}_health"
    health_result = run_command(health_cmd, "Reward health analysis")
    
    return all([diff_result.returncode == 0, det_result.returncode == 0, health_result.returncode == 0])
...
def main():
    ...
    # Success criteria
    success = (
        training_result.returncode == 0 and
        outputs_found >= 3 and  # At least 3 outputs should be generated
        total_time < 180  # Under 3 minutes
    )
