...
def materialize_samsum(output_dir: Path, max_samples: int = 50) -> str:
    ...
    for i, example in enumerate(train_data):
        ...
        summary = f"Category: {label_names[example['label']]}. {example['text'][:100]}..."
...
def materialize_hh(output_dir: Path, max_samples: int = 50) -> str:
    ...
    for i, example in enumerate(train_data):
        ...
        safety_score = 0.8 if example["label"] == 1 else 0.3  # Positive reviews are safer
...
def materialize_mbpp(output_dir: Path, max_samples: int = 50) -> str:
    ...
    for i, example in enumerate(train_data):
        ...
        prompt = f"Generate a Python function to analyze this text: {example['text'][:100]}"
    ...
    print(f"Created AG News Code manifest with {len(manifest)} samples: {manifest_path}")
    return str(manifest_path)

def main():
    parser = argparse.ArgumentParser(description="Materialize dataset manifests")
    parser.add_argument("--output-dir", type=str, default="reference/datasets", 
