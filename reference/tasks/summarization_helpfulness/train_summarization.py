...
class MockGPT2Model:
    ...
    def generate(self, input_ids, max_length=50, **kwargs):
        ...
        new_tokens = torch.randint(0, self.vocab_size, (batch_size, max_length - seq_len))
...
class RewardModel:
    ...
    def compute_reward(self, summary: str, preference_score: float) -> float:
        ...
        saturated_reward = torch.tanh(scaled_reward) if hasattr(torch, 'tanh') else scaled_reward
...
def train_summarization(
    ...
    print(f"🚀 Starting summarization helpfulness training")
    ...
    for step in range(num_steps):
        ...
        rewards = []
        for item in batch:
            reward = reward_model.compute_reward(item["summary"], item["preference_score"])
        ...
        step_metrics = {
            "step": step,
            "reward_mean": float(torch.tensor(rewards).mean()) if TORCH_AVAILABLE else sum(rewards) / len(rewards),
            "reward_std": float(torch.tensor(rewards).std()) if TORCH_AVAILABLE else 0.1,
        ...
        # Print progress
        if step % 10 == 0:
            print(f"Step {step:3d}: Reward={step_metrics['reward_mean']:.3f}, "
    ...
    print(f"  1. KL divergence spike at step 47 (2.3x normal)")
    print(f"  2. Non-deterministic training (15% variance expected)")
    print(f"  3. Reward saturation (0.87 saturation score expected)")
    
    return training_metrics

def main():
    parser = argparse.ArgumentParser(description="Train summarization helpfulness with RLHF")
    parser.add_argument("--model", default="125M", choices=["125M", "355M", "774M"], help="Model size")
    parser.add_argument("--learning_rate", type=float, default=1e-4, help="Learning rate")
    parser.add_argument("--steps", type=int, default=100, help="Number of training steps")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--output_dir", default="summarization_outputs", help="Output directory")
    ...
    print(f"  rldk diff --a {args.output_dir} --b {args.output_dir}_run2 --signals kl_divergence,reward_mean")
