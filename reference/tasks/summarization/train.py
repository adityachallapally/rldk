        ...
        for step in range(max_steps):
            ...
            if input_length != target_length:
                ...
                if input_length < target_length:
                    ...
                    padding = torch.full((1, padding_length), tokenizer.eos_token_id, dtype=torch.long)
                    ref_inputs["input_ids"] = torch.cat([ref_inputs["input_ids"], padding], dim=1)
            ...
            log_entry = {
                ...
                "tokenizer.merges_sha256": compute_sha256(str(tokenizer.merge_file) if hasattr(tokenizer, 'merge_file') else "none"),
                "model.name": model.name_or_path,
                "model.revision": "main",  # Default for local models
                "env.python_version": platform.python_version(),
                "env.torch_version": torch.__version__,
                "env.platform": platform.platform(),
                "inputs.input_ids_sha256": compute_sha256(str(inputs["input_ids"].numpy().tobytes())),
                "inputs.attention_mask_sha256": compute_sha256(str(inputs["attention_mask"].numpy().tobytes())),
            ...
            if step % 10 == 0:
                print(f"Step {step}: Loss = {loss.item():.4f}, Reward = {reward_scalar:.4f}")
...
def main():
    ...
    parser.add_argument("--pad-direction", type=str, default="right",
                       choices=["left", "right"],
                       help="Padding direction")
    parser.add_argument("--truncate-at", type=int, default=None,
