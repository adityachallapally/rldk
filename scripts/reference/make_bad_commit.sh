...
cp scripts/tasks/summarization/train.py scripts/tasks/summarization/train.py.backup

# Make the change - flip pad direction from right to left
echo "Modifying summarization training script..."
sed -i 's/pad_direction: str = "right"/pad_direction: str = "left"/' scripts/tasks/summarization/train.py

# Also change the truncate_at default to create more divergence
sed -i 's/truncate_at: Optional\[int\] = None/truncate_at: Optional\[int\] = 256/' scripts/tasks/summarization/train.py

# Stage the changes
git add scripts/tasks/summarization/train.py
...
echo "  # or"
echo "  cp scripts/tasks/summarization/train.py.backup scripts/tasks/summarization/train.py"
echo ""
