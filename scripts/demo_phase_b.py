...
def create_synthetic_data():
    ...
    data['reward_mean'] = data['tokens_out'] * 0.01 + np.random.normal(0.5, 0.1, n_steps)
    ...
    data['reward_mean'] = np.random.uniform(0.4, 0.6, n_steps)  # Override for calibration test
...
def create_reference_data():
    ...
    return pd.DataFrame({
        'step': range(n_steps),
        'reward_mean': np.random.normal(0.5, 0.15, n_steps),  # Different distribution
        'reward_std': np.random.uniform(0.1, 0.3, n_steps)
    })
...
def main():
    ...
    try:
        # Demo reward health
        health_report = demo_reward_health()
