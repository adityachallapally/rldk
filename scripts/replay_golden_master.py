...
def compare_json_objects(current: Any, expected: Any, path: str = "") -> list[tuple[str, Any, Any]]:
    ...
    if isinstance(current, dict):
        ...
        if len(current) != len(expected):
            ...
            for i, (curr_item, exp_item) in enumerate(zip(current, expected, strict=False)):
...
def compare_command_results(
    ...
    if len(expected_artifacts) != len(current_artifacts):
        ...
        for i, (current_artifact, expected_artifact) in enumerate(zip(current_artifacts, expected_artifacts, strict=False)):
