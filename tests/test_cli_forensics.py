...
def test_env_audit_help():
    """Test env-audit help."""
    result = subprocess.run(["rldk", "env-audit", "--help"], capture_output=True, text=True)
    assert result.returncode == 0
...
def test_log_scan_help():
    """Test log-scan help."""
    result = subprocess.run(["rldk", "log-scan", "--help"], capture_output=True, text=True)
    assert result.returncode == 0
...
def test_diff_ckpt_help():
    """Test diff-ckpt help."""
    result = subprocess.run(["rldk", "diff-ckpt", "--help"], capture_output=True, text=True)
    assert result.returncode == 0
...
def test_reward_drift_help():
    ...
    result = subprocess.run(["rldk", "reward-drift", "--help"], capture_output=True, text=True)
    assert result.returncode == 0
...
def test_doctor_help():
    """Test doctor help."""
    result = subprocess.run(["rldk", "doctor", "--help"], capture_output=True, text=True)
    assert result.returncode == 0
...
def test_compare_runs_help():
    ...
    result = subprocess.run(["rldk", "compare-runs", "--help"], capture_output=True, text=True)
    assert result.returncode == 0
