...
def test_first_divergence_detection():
    ...
    if drift_json.exists():
        ...
        assert first_step <= 5, f"First divergence should be early, but was at step {first_step}"
        
        # Check that signals were tripped
        tripped_signals = data.get("tripped_signals", [])
        assert len(tripped_signals) > 0, "No signals were tripped during divergence detection"
...
def test_divergence_reproducibility():
    ...
    if drift_json.exists():
        ...
        assert "repro" in data or "repro_command" in data or "details" in data, \
...
def test_divergence_analysis_outputs():
    ...
    for filename in expected_files:
        ...
        assert file_path.exists(), f"Expected divergence analysis file {filename} does not exist"
...
def test_divergence_timing():
    ...
    if drift_json.exists():
        ...
        assert first_step <= 10, f"Divergence should be detected early, but was at step {first_step}"
