...
from rldk.io.event_schema import Event, create_event_from_row, events_to_dataframe, dataframe_to_events
...
class TestDeterminismCard:
    ...
    def test_generate_determinism_card(self, sample_events, tmp_path):
        ...
        try:
            card_data = generate_determinism_card(sample_events, run_path, str(tmp_path))
    ...
    def test_generate_drift_card(self, sample_events, divergent_events, tmp_path):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.jsonl', delete=False) as f1:
            with tempfile.NamedTemporaryFile(mode='w', suffix='.jsonl', delete=False) as f2:
                # Write dummy data
                for i in range(5):
                    f1.write(json.dumps({'step': i, 'reward_mean': 0.5 + i*0.1}) + '\n')
                    f2.write(json.dumps({'step': i, 'reward_mean': 0.5 + i*0.1 + 0.5}) + '\n')
                run_a_path = f1.name
                run_b_path = f2.name
        
        try:
            card_data = generate_drift_card(sample_events, divergent_events, run_a_path, run_b_path, str(tmp_path))
...
class TestCardIntegration:
    ...
    def test_card_consistency(self, sample_events, tmp_path):
        ...
        try:
            ...
            det_card = generate_determinism_card(sample_events, run_path, str(tmp_path / "det"))
            
            # Generate reward card
            reward_card = generate_reward_card(sample_events, run_path, str(tmp_path / "reward"))
    ...
    def test_card_with_identical_runs(self, sample_events, tmp_path):
        ...
        try:
            ...
            card_data = generate_drift_card(sample_events, sample_events, run_a_path, run_b_path, str(tmp_path))
...
class TestCardEdgeCases:
    ...
    def test_empty_events(self, tmp_path):
        ...
        try:
            ...
            det_card = generate_determinism_card(empty_events, run_path, str(tmp_path / "det"))
            reward_card = generate_reward_card(empty_events, run_path, str(tmp_path / "reward"))
    ...
    def test_single_event(self, tmp_path):
        ...
        try:
            ...
            det_card = generate_determinism_card([single_event], run_path, str(tmp_path / "det"))
            reward_card = generate_reward_card([single_event], run_path, str(tmp_path / "reward"))
