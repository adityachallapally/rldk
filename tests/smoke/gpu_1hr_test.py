...
def run_comprehensive_rldk_analysis(task_outputs: Dict[str, str]) -> Dict[str, bool]:
    ...
    for task_name, output_dir in task_outputs.items():
        ...
        analyses = [
            ...
            (
                "determinism",
                f"rldk check-determinism --cmd 'python scripts/tasks/{task_name}/train_*.py --steps 10' --compare reward_mean --output-dir {output_dir}_determinism",
            ),
            (
                "reward_health",
...
def main():
    ...
    tasks = {
        "summarization_helpfulness": {
            "cmd": "python scripts/tasks/summarization_helpfulness/train_summarization.py --steps 100 --output_dir gpu_test_summarization",
            "timeout": 1200,  # 20 minutes
        },
        "refusal_safety": {
            "cmd": "python scripts/tasks/refusal_safety/train_refusal_safety.py --steps 200 --output_dir gpu_test_safety",
            "timeout": 1800,  # 30 minutes
        },
        "code_fix_prompts": {
            "cmd": "python scripts/tasks/code_fix_prompts/train_code_fix.py --steps 300 --output_dir gpu_test_code",
