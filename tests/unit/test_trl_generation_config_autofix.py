...
def stub_trl_environment(monkeypatch) -> Dict[str, object]:
    ...
    DummyTokenizer.loaded = []
    DummyValueHead.loaded = []
...
def test_create_ppo_trainer_adds_value_model_without_reloading_policy(stub_trl_environment):
    """Missing value model should not trigger redundant policy/ref loads."""
    ...
    assert not caught, "Auto-fix should be silent when value model is inferred"

    assert trainer.model is policy
    assert trainer.ref_model is reference
    assert trainer.reward_model is reward
    assert trainer.value_model is not None
    assert trainer.value_model is not policy
    assert trainer.value_model is not reference
    assert trainer.value_model is not reward
    ...
    # Only the value model should be instantiated via from_pretrained
    assert env["model_cls"].loaded == ["value-only-model"]

    fix_targets = set(env["fix_calls"])
    assert {"policy-model", "reference-model", "reward-model", "value-only-model"}.issubset(
        fix_targets
    )
