...
class TestDeterminismHarness:
    ...
    def test_identical_runs_pass(self):
        ...
        with patch('subprocess.run', return_value=mock_result), \
             patch('rldk.determinism.check._run_deterministic_cmd', return_value=mock_result):
            report = check(
                cmd="python train.py",
    ...
    def test_nondeterministic_runs_fail(self):
        ...
        for i in range(3):
            ...
            modified_data['reward_mean'] = [v + 0.001 * i for v in modified_data['reward_mean']]
            modified_data['kl_mean'] = [v + 0.0001 * i for v in modified_data['kl_mean']]
    ...
    def test_nondeterministic_ops_detection(self):
        ...
        with patch('subprocess.run', side_effect=[mock_result1, mock_result2]), \
             patch('rldk.determinism.check._run_deterministic_cmd', side_effect=[mock_result1, mock_result2]):
            report = check(
                cmd="python train.py",
                compare=['reward_mean'],
                replicas=2,
    ...
    def test_replica_variance_calculation(self):
        ...
        with patch('subprocess.run', side_effect=mock_results), \
             patch('rldk.determinism.check._run_deterministic_cmd', side_effect=mock_results):
            report = check(
                cmd="python train.py",
